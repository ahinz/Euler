...
object Solution11 {
  ...
  val lst = List(List(8,2,22,97,38,15,0,40,0,75,4,5,7,78,52,12,50,77,91,8),
...
List(1,70,54,71,83,51,54,69,16,92,33,48,61,43,52,1,89,19,67,48))
  ...
  def sum4(v: List[Int], sMax:Int = 0 ):Int = v match {
    case Nil => sMax
    case _ => sum4(v.tail, sMax.max(v.take(4).reduceLeft(_*_)))
  }

  def transp1col(v: List[List[Int]], newRow: List[Int] = Nil, newTable: List[List[Int]] = Nil):(List[Int], List[List[Int]]) =
                   if (v.filter(_.length > 0).length == 0)
                     (newRow.reverse, newTable.reverse)
                   else 
                     transp1col(v.tail, v.head.head :: newRow, v.head.tail :: newTable)


  def transp(v: List[List[Int]], newTable: List[List[Int]] = Nil):List[List[Int]]  = 
    if (v.length == 0 || v(0).length == 0) newTable.reverse
    else 
      transp1col(v) match {
        case (newRow, trimmedTable) => transp(trimmedTable, newRow :: newTable)
      }


  def shift(samt:Int, pad:Int, lst:List[Int]) =
    ((for(k <- 0 until (pad - samt)) yield(1)) ++
    lst ++
    (for(k <- 0 until samt) yield(1))).toList

  def shiftAll(v: List[List[Int]]) =
    v.zip(0.until(v.length)).map(_ match {
                                 case (lst, shft) => shift(shft, lst.length - 1, lst)
    })

  val maxHsum = lst.map(sum4(_)).max
  val maxVsum = transp(lst).map(sum4(_)).max
  val maxDsum = transp(shiftAll(lst)).map(sum4(_)).max
  val maxDsum2 = transp(shiftAll(lst.map(_.reverse)).map(_.reverse)).map(sum4(_)).max

  def run = println(List(maxHsum,maxVsum,maxDsum,maxDsum2).max)
